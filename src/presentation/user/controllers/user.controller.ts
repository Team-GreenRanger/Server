...
export class UserController {
  ...
  async getStatistics(@Request() req: any): Promise<UserStatisticsResponseDto> {
    ...
    const totalCo2ReductionNum = missions.reduce((total, mission) => {
      // mission과 co2ReductionAmount가 존재하고 숫자인지 확인
      const co2Amount = mission?.co2ReductionAmount;
      const numericAmount = typeof co2Amount === 'number' ? co2Amount : Number(co2Amount) || 0;
      return total + numericAmount;
    }, 0);

    // CO2 감소량을 문자열로 포맷팅 (kg 단위, 소수점 2자리)
    // Number 타입임을 명시적으로 확인
    const totalCo2Reduction = Number(totalCo2ReductionNum).toFixed(2);
